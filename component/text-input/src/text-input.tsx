...
const RNTextInput =
  Platform.OS === "web" ? GenericTextInput : TextInputWithGestureSupport;
...
function TextInputComponent(props: TextInputProps, ref?: React.Ref<any>) {
  ...
  const fillStyle: any = fill
    ? Platform.select({ web: { flex: "auto" } })
    : undefined;
  const content = (
  ...
  if (label) {
    return (
      <View style={[styles.outerContainer, fillStyle]}>
        <Text style={[labelColorStyle, s.f5, { fontWeight: "bold" }]}>{label}</Text>
        <View style={styles.spacer} />
        {content}
      </View>
    );
