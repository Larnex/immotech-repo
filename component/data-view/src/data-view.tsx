...
function isValidEmptyMessageNode(
  emptyMessage: EmptyMessageType
): emptyMessage is ReactNode {
...
function isEmptyMessageObject(
  ...
  return (
    ...
    !!(emptyMessage as EmptyMessageObjectType)?.title
  );
}

function hasEmptyData<T, P extends { items: T[] }>(data: T | T[] | P) {
  return (
    !data || (data as T[])?.length === 0 || (data as P)?.items?.length === 0
  );
}
